...
> - `/etc/hostname`, `/etc/mkinitcpio.conf` and `linux.preset` are no longer
>   tracked or restored. Each machine keeps its own boot image setup.
...
- **limine** — Gruvbox bootloader theme (`theme.conf`) plus `install.sh`, which
  ...
  With `limine-mkinitcpio-hook` installed, kernel builds go through
  ...
  and `limine-entry-tool` registers them in `limine.conf`. To rebuild, use
  `sudo limine-mkinitcpio`, not `mkinitcpio -P`.
...
- `/etc/pacman.conf` is **not** copied in — it is otherwise stock. `restore.sh`
  ...
  `scripts/packages/pins.txt`. Safe to re-run. `scripts/packages/install.sh`
  also uncomments `[multilib]` if it is off.
- `/etc/mkinitcpio.conf` and `/etc/mkinitcpio.d/linux.preset` are **not**
  copied in. The Arch installer writes them to fit the machine's disk setup
  (an encrypted disk adds a hook, a different bootloader wants different
  images), and overwriting them can leave it unbootable. On a Limine machine,
  rebuild with `sudo limine-mkinitcpio`; otherwise `sudo mkinitcpio -P`.
